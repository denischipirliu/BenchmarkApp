...
void Blowfish_ECB_encrypt(blf_ctx* c, unsigned char* input, unsigned char* output, int length) {
	unsigned long xl, xr;
	for (int i = 0; i < length; i += 8) {

		xl = ((unsigned long*)input)[i / 8];
		xr = ((unsigned long*)input)[i / 8 + 1];

		Blowfish_encipher(c, &xl, &xr);

		((unsigned long*)output)[i / 8] = xl;
		((unsigned long*)output)[i / 8 + 1] = xr;
	}
}

void Blowfish_ECB_decrypt(blf_ctx* c, unsigned char* input, unsigned char* output, int length) {
	unsigned long xl, xr;
	for (int i = 0; i < length; i += 8) {
